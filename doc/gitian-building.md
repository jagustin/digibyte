...
*Setup instructions for a Gitian build of LekCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the LekCoin
...
- [Building LekCoin Core](#building-lekcoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-lekcoin-host/README.md).
...
Clone the git repositories for lekcoin and Gitian.
...
in the lekcoin repository under 'Fetch and create inputs' to install sources which require
...
To build LekCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the lekcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/lekcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/lekcoin/lekcoin
...
For example:
```bash
URL=https://github.com/laanwj/lekcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit lekcoin=${COMMIT} --url lekcoin=${URL} ../lekcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit lekcoin=${COMMIT} --url lekcoin=${URL} ../lekcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit lekcoin=${COMMIT} --url lekcoin=${URL} ../lekcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the lekcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../lekcoin/contrib/gitian-descriptors/*|sort|uniq )
...
LEKPATH=/some/root/path/lekcoin
SIGPATH=/some/root/path/lekcoin-detached-sigs

./bin/gbuild --url lekcoin=${LEKPATH},signature=${SIGPATH} ../lekcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/lekcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/lekcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/lekcoin-osx-build.assert
```
...
[lekcoin-core/gitian.sigs](https://github.com/lekcoin-core/gitian.sigs/) repository, or if that's not possible create a pull

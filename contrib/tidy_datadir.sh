#!/bin/bash
# Copyright (c) 2013 The LekCoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete LekCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no LekCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old LekCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected LekCoin 0.7 datadir."
    ;;
  3)
    echo "Detected LekCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected LekCoin 0.8 datadir."
    ;;
esac

...
The RPM spec file provided here is for LekCoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://lekcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[LekCoin git repository](https://github.com/lekcoin/lekcoin) but are not part of
...
Currently only `Source10` is used, for the example `lekcoin.conf` file.
...
in the lekcoin git repository. At present only an SVG version of the LekCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the lekcoin
...
needed when 0.12.1 is released, a proper fix is already in the LekCoin git
master branch.
...
idea to build LekCoin Core against a static version of that library compiled
...
The `lekcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `lekcoin-server` package. At this point, most RPM based
...
The `lekcoin-server` package should have SELinux support. How to properly do
...
LekCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/lekcoin/). He has been
packaging LekCoin for Fedora at least since 2012.
...
4. I renamed the `lekcoin` package that contains the Qt GUI to `lekcoin-core` as
that appears to be how the general population refers to it, in contrast to
`lekcoin-xt` or `lekcoin-classic`. I wanted to make sure the general population

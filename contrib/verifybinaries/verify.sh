...
###   lekcoincore.org and lekcoin.org and compares them.
...
WORKINGDIR="/tmp/lekcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://lekcoincore.org"
HOST2="https://lekcoin.io"
BASEDIR="/bin/"
VERSIONPREFIX="lekcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'lekcoin-',
...
if [ $? -ne 0 ]; then
   echo "lekcoin.org failed to provide signature file, but lekcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "lekcoin.org and lekcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the LekCoin Core binary release signing key installed?"
   fi

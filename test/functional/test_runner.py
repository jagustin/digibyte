...
`test/functional/test_framework/test_framework.py:LekCoinTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/lekcoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_lekcoind = config["components"].getboolean("ENABLE_DIGIBYTED")
    ...
    if not (enable_wallet and enable_utils and enable_lekcoind):
        print("No functional tests to run. Wallet, utils, and lekcoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    # Warn if lekcoind is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "lekcoind"]) is not None:
            print("%sWARNING!%s There is already a lekcoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "DIGIBYTED" not in os.environ:
        os.environ["DIGIBYTED"] = build_dir + '/src/lekcoind' + exeext
        os.environ["DIGIBYTECLI"] = build_dir + '/src/lekcoin-cli' + exeext
